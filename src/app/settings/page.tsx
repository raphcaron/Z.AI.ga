...
export default function SettingsPage() {
  ...
  const { user, isSubscribed, loading: authLoading, refreshSubscription } = useAuth();
  ...
  // Reload favorites when the favorites set changes (e.g., when removing from card)
  useEffect(() => {
    if (user && !loadingFavorites) {
      loadFavoriteSessions();
    }
  }, [favorites.size]);
      ...
      if (enabled) {
        ...
        if (existing?.id) {
          ...
          const { error } = await supabase
            ...
            .update({
              status: 'active',
              stripe_price_id: 'price_monthly_dev',
        ...
        setMessage({ type: 'success', text: 'Premium activated! You now have full access.' });
      } else {
        // Cancel subscription
        const { error } = await supabase
          .from('subscriptions')
        ...
        if (error) throw error;
        setMessage({ type: 'success', text: 'Premium deactivated.' });
      }
                      ...
                      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {favoriteSessions.map((fav) => (
                          <SessionCard 
                            ...
                            session={{
                              id: fav.sessions?.id || '',
                              title: fav.sessions?.title || 'Unknown Session',
                              slug: fav.sessions?.id || '',
                              description: null,
                              thumbnail: fav.sessions?.thumbnail,
                              duration: fav.sessions?.duration || 0,
                              difficulty: (fav.sessions?.difficulty || 'beginner') as 'beginner' | 'intermediate' | 'advanced',
                              instructor: fav.sessions?.instructor,
                            }}
                          />
                        ))}
                      </div>
              ...
              <TabsContent value="subscription">
                <Card className="rounded-3xl">
                  ...
                  <CardContent>
                    ...
                    <div className="mt-8 p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800">
                      <div className="flex items-center gap-2 mb-3">
                        <Sparkles className="w-5 h-5 text-amber-600 dark:text-amber-400" />
                        <h4 className="font-semibold text-amber-800 dark:text-amber-200">Dev Tools</h4>
                      </div>
                      <p className="text-sm text-amber-700 dark:text-amber-300 mb-4">
                        Quick toggle to simulate premium subscription for testing.
                      </p>
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="font-medium text-amber-900 dark:text-amber-100">Simulate Premium</p>
                          <p className="text-xs text-amber-600 dark:text-amber-400">
                            {isSubscribed ? 'Premium active' : 'Premium inactive'}
                          </p>
                        </div>
                        <Switch
